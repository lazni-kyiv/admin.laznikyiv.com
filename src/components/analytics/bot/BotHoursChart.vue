<template>
  <AnalyticsCard title="Активність бота по годинах" icon="clock-white">
...
const chartOption = computed(() => {
  ...
  const booking = Array(24).fill(0)
  const call = Array(24).fill(0)

  props.botData.forEach(e => {
    const hour = new Date(e.timestamp).getUTCHours()
    if (e.event === 'booking') booking[hour]++
    if (e.event === 'call') call[hour]++
  })
