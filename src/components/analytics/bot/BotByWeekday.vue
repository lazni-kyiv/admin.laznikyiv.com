<template>
  <AnalyticsCard title="Активність по днях тижня" icon="calendar">
...
const option = computed(() => {
  const counters = {
    call: Object.fromEntries(WEEKDAYS.map((d) => [d, 0])),
    booking: Object.fromEntries(WEEKDAYS.map((d) => [d, 0])),
  };

  botData.value.forEach((e) => {
    const d = new Date(e.timestamp);
    const day = WEEKDAYS[(d.getDay() + 6) % 7];
    counters[e.event][day]++;
  });
  ...
  return {
    ...
    series: EVENTS.map((e) => ({
      ...
      data: WEEKDAYS.map((d) => counters[e][d]),
